%% D2_jos_sync2_Epoching_steps_delta_preAC
% Pre-Artifact corrected data dor comparison graph

clear all; close all; clc;
MAINPATH = 'C:\Users\ebmi2273\jos_sync2\';
addpath([MAINPATH, 'eeglab2020_0\']);
PATHIN = [MAINPATH, 'rawdata\ana2_A3_HS\'];
PATHOUT = ['C:\Users\ebmi2273\jos_sync\data\ana1_D2_Epoch_steps_brain_reref_preAC\']; %different version to rereference raw data
mkdir(PATHOUT);
cd(MAINPATH);
...
for i_sub = 1:length(SUBJ)
    ...
    EEG = pop_loadset([PATHIN, 'jos_sync', SUBJ{i_sub}, '_' TYPE{i_sub}, '_steps.mat']);
 ...
 %   no jointprob here because this removes artifact. 
    ...
    for e = sepevs(i_sepevs,1):sepevs(i_sepevs,3)%1:length(EVENTS)
        ...
        EEG = pop_saveset(EEG, 'filename',[EEG.setname, '_preAC.set'],...
            'filepath',PATHOUT);
        EEG = eeg_checkset(EEG);
