%% F3_jos_sync2_ComputeERSP
% make the post AC ERSP
% This version is timed to the experimenter's steps so we can show it to
% the reviewer and maybe include in the paper???
...
PATHOUT = [MAINPATH, 'data\ana3_processed\ERSP4_expsteps\'];
mkdir(PATHOUT);
ETYPE = {'act', 'pas'};
...
for i_cond = 1:length(CONDS)
    for i_sub = 1:size(ALLSUB,1)
        i_human = 2; %1 = par, 2 = exp

            i_elec = 1;
            EEG1 = pop_loadset('filename', [PATHIN,'jos_sync2', ALLSUB{i_sub}, '_', ETYPE{i_elec}, '_EEG_epoched_rejected_', HUMANS{i_human},CONDS{i_cond}, '.set']);
            i_elec = 2;
            EEG2 = pop_loadset('filename', [PATHIN,'jos_sync2', ALLSUB{i_sub}, '_', ETYPE{i_elec}, '_EEG_epoched_rejected_', HUMANS{i_human},CONDS{i_cond}, '.set']);
            EEG = pop_mergeset(EEG1, EEG2);
            EEG.delta = []; % not sure how to concatonate & resort these values after merging so I just remove them. I use the delta values in EEG.epoch later instead
           ...
           %% ERSP %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            % again, not used in manuscript. will probably delete later. 
    ...
    % save data
    save([PATHOUT, 'TF5_680both_Exp', '_', HUMANS{i_human}], 'TF', '-v7.3'); 
    save([PATHOUT, 'step_trials'], 'tcounter');
    save([PATHOUT, 'numEpochsGait'], 'numEpochsGait'); 
end
