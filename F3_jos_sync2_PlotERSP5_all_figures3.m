...
subplot(2, 6, 5:6)
h1 = raincloud_plot(perc_perc_freqlocked(:,3), 'box_on', 1, 'color', [.85       .54       .765], 'alpha', 0.5,...
    'box_dodge', 1, 'box_dodge_amount', .5, 'dot_dodge_amount', .5, 'box_col_match', 1);%'density_type', 'rash'
h2 = raincloud_plot(perc_perc_freqlocked(:,1), 'box_on', 1, 'color', [0.4000    0.6510    0.1176], 'alpha', 0.5,...
    'box_dodge', 1, 'box_dodge_amount', 1.5, 'dot_dodge_amount', 1.5, 'box_col_match', 1);
h3 = raincloud_plot(perc_perc_freqlocked(:,2), 'box_on', 1, 'color', [0.4588    0.4392    0.7020], 'alpha', 0.5,...
...
subplot(2, 6, 11:12)
h11 = raincloud_plot2(perc_inphase_lock(:,3).*100, 'box_on', 1, 'color', [.85       .54       .765], 'alpha', 0.5,...
    'box_dodge', 1, 'box_dodge_amount', 2.76, 'dot_dodge_amount', 2.76, 'box_col_match', 1);%'density_type', 'rash'
h22 = raincloud_plot2(perc_inphase_lock(:,1).*100, 'box_on', 1, 'color', [0.4000    0.6510    0.1176], 'alpha', 0.5,...
    'box_dodge', 1, 'box_dodge_amount', 8.1, 'dot_dodge_amount', 8.1, 'box_col_match', 1);
h33 = raincloud_plot2(perc_inphase_lock(:,2).*100, 'box_on', 1, 'color', [0.4588    0.4392    0.7020], 'alpha', 0.5,...
...
fnsize = 10;
cluschansr = [25 58 56 22];
cluschansl = [40 8  11 42];
neworder = [2 1 3];
clim = .5;
part = {'mean', 'abs'};
...
% row 2: Difference Topos ROI averaged over time
tfranges = [ 16 32];
panel = 1;
...
for i_band = 1%:length(tfranges)
    for i_col = 1:3;
        ...
        sbplot(3,3,3+ panel) % starts at second row because supertitle warps the top row for some reason
        topoplot(tmp, TF.chanlocs, 'maplimits', [-clim clim],'emarker2',{[starvals],'s','k'} );
        title([CONDS_long{1}, '-',CONDS_long{1+i_col} ]);
        panel = panel+1;
    end
...
for i_cond = 1:3;
    for i_band = 1%:length(tfranges)
        ...
        sbplot(3,3,6+ 3*(i_band)+i_cond-3) % starts at second row because supertitle warps the top row for some reason
        topoplot(tmp, TF.chanlocs, 'maplimits', [-clim clim] ,'emarker2',{[starvals],'s','k'} );
