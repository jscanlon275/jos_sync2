...
%       TTL.signal['BrainVision''|'accelerometer'] - type of stream
%
...
%   triggerType - 'none'. for recordings with BrainRecorder Indicate how  LiveAmp trigges were
...
for s = 1:numAcc
 ...
 if strcmp(Acc.TTL. signal, 'BrainVision') %if there are multiple EEG datasets the last one is chosen
        idxEEG(end+1) = s;
        srate = Acc.srate;
        idxLA(end+1) = s;
        % error if not enough info is availabe to detect LiveAmp triggers
        if ~exist('triggerType','var') ||~any(strcmp({'BrainVision'}, triggerType))
...
for s = 1:numAcc % loop through all structures
    ...
    % load template
   if strcmp(Acc.TTL.signal, 'BrainVision')
        skip = 1;
    elseif strcmp(Acc.TTL.signal, 'accelerometer')
        load([PATHIN, 'tmpAcc'],'tmpAcc');
    else
        error('Your TTL.signal is not of type "BrainVision" or " accelerometer".');
    end
    ...
    if ~skip
        ...
        if fastDetection
            ...
            if strcmp(Acc.TTL. signal, 'accelerometer')
        ...
        %% LiveAmp recorded with brainVision recorder 
    elseif strcmp(Acc.TTL.signal, 'BrainVision')
              
        [Acc.event(strcmp({Acc.event.type}, 'M  1')).type] = deal('sync');
        lat = [Acc.event(strcmp({Acc.event.type}, 'sync')).latency];   % latencies of sync triggers
...
for a = 1:numAcc
    ...
    try allAcctmp{a}.event= rmfield( allAcctmp{a}.event,{'channel','bvtime','bvmknum','code','urevent', 'visible'}); end
